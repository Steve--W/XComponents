...
uses
  ...
  NodeJSFS,
  {$endif}
  SysUtils, Classes, xPasTree, xPScanner;
...
Var
  XIDEProcsList:array of TUserProcItem;
  XIDEProjectDir:String;
...
procedure TPasParser.ParseProcedureOrFunctionHeader(Parent: TPasElement;
  ...
  procedure XIDEUnmangleSourceLineNumber(LineNumber: integer; out
    Line, Column: integer);
  begin
    Line:=Linenumber;
    Column:=0;
...
//    Writeln('Done: ',TokenInfos[Curtoken],' ',CurtokenString);
  ...
  begin
    ...
    begin
      ...
      pitem.FileName:=rightstr(Parent.SourceFilename,length(Parent.SourceFilename)-length(tinc));
      XIDEUnmangleSourceLineNumber(Parent.SourceLinenumber,aRow,aColumn);
      pitem.LineNum := aRow;
